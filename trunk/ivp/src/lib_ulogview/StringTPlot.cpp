...
bool StringTPlot::setValue(double timestamp, 
			   const string& value,
			   const string& source,
			   const string& srcaux)
{
...
string StringTPlot::getSourceByIndex(unsigned int index) const
{
  if(index < m_time.size())
...
//---------------------------------------------------------------
// Procedure: getSrcAuxByIndex

string StringTPlot::getSrcAuxByIndex(unsigned int index) const
{
  if(index < m_time.size())
    return(m_srcaux[index]);
  return("");
}
