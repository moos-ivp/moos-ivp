...
{
#if 1
  cout << "CMOOSBridge:: In OnNewMail!!!!!!" << endl;
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
    ...
    cout << "CMOOSBridge::Found Some Mail!!!!" << endl;
    
    // Example String "var,community,host,port,alias"
    if(moosvar == "PMB_REGISTER") {
      cout << "Handling a Dynamic Registration!!!!" << endl;
	...
	if(left == "src_var")
	  src_var = right;
	else if(left == "dest_community")
	  dest_community = right;
	else if(left == "dest_host")
	  dest_host = right;
	else if(left == "dest_port")
	  dest_port = right;
	else if(left == "dest_alias")
	  dest_alias = right;
      }
      string sSrcCommunity = m_sLocalCommunity ;
      string sLocalHost = "LOCALHOST";
