...
bool CRS_Model::setParam(string param, string value)
{
  param = tolower(param);
...
bool CRS_Model::handleMsg(string key, double dval, string sval,
...
void CRS_Model::setCurrTime(double new_time)
{
...
vector<VarDataPair> CRS_Model::getMessages(bool clear)
{
  if(!clear)
    return(m_messages);
...
bool CRS_Model::handleNodeReport(const string& node_report_str)
{
  NodeRecord new_node_record = string2NodeRecord(node_report_str);

  if(!new_node_record.valid()) {
    addMessage("CRS_DEBUG", "Invalid incoming node report");
    return(false);
  }
...
bool CRS_Model::handleRangeRequest(const string& request)
{
  string vname;
  ...
  if(vname == "") {
    addMessage("CRS_DEBUG", "Range Request with null node name");
    return(false);
  }
  if(m_verbose) 
...
void CRS_Model::addMessage(const string& varname,
			   const string& value)
{
...
void CRS_Model::addMessage(const string& varname, double value)
{
  VarDataPair pair(varname, value);
  m_messages.push_back(pair);
}
...
void CRS_Model::postRangePulse(unsigned int ix, string color,
...
void CRS_Model::postNodeRangeReport(unsigned int rec_ix, 
				    unsigned int tar_ix, 
				    double actual_range)
{
  ...
  if((m_report_vars == "short") || (m_report_vars == "both")) {
    ...
    addMessage("CRS_RANGE_REPORT", full_str);
  }

  if((m_report_vars == "long") || (m_report_vars == "both"))
    addMessage("CRS_RANGE_REPORT_" + toupper(rec_name), str);
  ...
  if((m_rn_algorithm != "") && m_ground_truth) {
    ...
    if((m_report_vars == "short") || (m_report_vars == "both")) {
      string full_str = "vname=" + rec_name + "," + str;
      addMessage("CRS_RANGE_REPORT_GT", full_str);
    }
    
    if((m_report_vars == "long") || (m_report_vars == "both"))
      addMessage("CRS_RANGE_REPORT_GT_" + toupper(rec_name), str);
  }
...
double CRS_Model::getTrueNodeNodeRange(unsigned int src_ix, 
...
double CRS_Model::getNoisyNodeNodeRange(double true_range) const
{
...
void CRS_Model::report(string msg, string term_color)
{
  if(m_verbose)
...
void CRS_Model::printNodeRecords()
{
...
bool CRS_Model::updateNodeRecords(const NodeRecord& new_record)
{
...
bool CRS_Model::setReachDistance(string str)
{
...
bool CRS_Model::setReplyDistance(string str)
{
  string left  = biteStringX(str, '=');
  string right = tolower(str); 
...
bool CRS_Model::setPingWait(string str)
{
  string left  = biteStringX(str, '=');
...
bool CRS_Model::setReportVars(string str)
{
...
bool CRS_Model::setRandomNoiseAlgorithm(string str)
{
