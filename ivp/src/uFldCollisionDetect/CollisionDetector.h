...
class CollisionDetector : public AppCastingMOOSApp
{
 ...
 protected:
   ...
   // holds the most recent NodeReport of a given vehicle
   std::map <std::string,NodeRecord> m_moos_map;  
   // holds the CollisionRecord associated with a given vehicle pair
   ...
   // make range pulses
   void MakeRangePulse(double, double);
   void MakeCollisionRangePulse(double, double);
   void MakeNearMissRangePulse(double, double);
   void MakeCPAViolationRangePulse(double, double);

   bool storeVehicleModes(CollisionRecord&, string, string);
   // stores vehicle modes for given vehicles (CR, v1, v2)
