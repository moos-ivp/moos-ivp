...
AOF_HShadow::AOF_HShadow(IvPDomain gdomain)
  : AOF(gdomain)
{
...
bool AOF_HShadow::setParam(const string& param, double param_val)
{
  if(param == "cn_crs") {
    cn_crs = param_val;
    cn_crs_set = true;
...
double AOF_HShadow::evalBox(const IvPBox *b) const
{
  double eval_crs = 0;
  double eval_spd = 0;
...
double AOF_HShadow::metric(double cval, double sval) const
{
