...
AOF_R14::AOF_R14(IvPDomain domain, CPXEngine* engine)
...
{
  double osx = m_cpa_engine->osx();
  double osy = m_cpa_engine->osy();
  double cnx = m_cpa_engine->cnx();
  double cny = m_cpa_engine->cny();
  double cnv = m_cpa_engine->cnv();
  
  double bng_os_to_cn = relAng(osx, osy, cnx, cny);
  double bng_cn_to_os = relAng(cnx, cny, osx, osy);

  double os_right_turn = angle360(bng_os_to_cn + 90);
  double cn_right_turn = angle360(bng_cn_to_os + 90);
  
  CPXEngineThin cpa_engine(cny, cnx, cn_right_turn, cnv, osy, osx);

  double initial_bng_rate = cpa_engine.bearingRate(os_right_turn, cnv);
