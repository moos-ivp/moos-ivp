...
public:
  ...
  short Set6BitITURCharValue(unsigned char* pBuffer, 
			     int buffer_size_in_bytes, 
			     int start_bit, int num_bits, char* pValue);

  short Get6BitITURCharValue(unsigned char* pBuffer, 
			     int buffer_size_in_bytes, 
			     int start_bit, int num_bits, char* pValue);
  
  void FormatNMEAString(const char* pCommand, const char* pAisChannel, 
			short seq_message_id, 
			const unsigned char* pDataBuffer, 
			long buffer_size_in_bytes,
			long buffer_size_in_bits);
  
  void  GetNMEAString(char* destination, short destination_length, 
		      short string_index);

  bool  SetNMEAString(char* new_string, short new_string_length, 
		      short string_index);
  ...
  static void  GetControlInformation(const char* msg_string, 
				     ControlInfoType* control_info);

  static short Convert6BitBinaryToShort(unsigned char msg_type);
  
  unsigned long GetUnsignedLongValue(unsigned char* pBuffer, 
				     int buffer_size_in_bytes, 
				     int start_bit, int num_bits);	
  long GetSignedLongValue(unsigned char* pBuffer, 
			  int buffer_size_in_bytes, 
			  int start_bit, int num_bits);	
 ...
 protected:	
  short	SetBit(unsigned char* pBuffer, int buffer_size_in_bytes, 
	       int which_bit);
  short	ClearBit(unsigned char* pBuffer, int buffer_size_in_bytes, 
		 int which_bit);
  long	ConvertCharacterTo6Bits(char inChar);
  short	ConvertDataBufferTo6BitString(const unsigned char* pDataBuffer, 
				      long buffer_size_in_bits,
				      char* pSentenceString, 
				      long sentence_size);
  unsigned char Get6BitCharAt(const unsigned char* pBuffer, 
			      int buffer_size_in_bytes, int start_bit);
