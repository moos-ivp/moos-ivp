...
int main(int argc, char *argv[])
{
  ...
  // Build all the ipf_plots from the vector of alog files.
  //---------------------------------------------------------------------
  vector<IPF_Plot> ipf_plots;

  parse_timer.reset();
  parse_timer.start();
  cout << "Parsing alog files to build IPF_Plots..." << endl;

  for(i=0; i<alog_files.size(); i++) {
    Populator_IPF_Plot pop_ipfp;
    pop_ipfp.set_vname("V_" + intToString(i)); 
    pop_ipfp.populate(alog_files[i]);
    pop_ipfp.print();

    int psize = pop_ipfp.size();
    for(int plot_ix=0; plot_ix<psize; plot_ix++) {
      IPF_Plot ipf_plot = pop_ipfp.getPlotIPF(plot_ix);
      ipf_plots.push_back(ipf_plot);
    }
  ...
  parse_timer.stop();
  cout << "Done: IPF_Plot parse time: " << parse_timer.get_float_cpu_time();
  cout << endl << endl;
  ...
  // Populate the GUI with the IPF_Plots built above
  cout << "MAIN - ipf_plot_size: " << ipf_plots.size() << endl;
  for(i=0; i<ipf_plots.size(); i++)
    gui->addIPF_Plot(ipf_plots[i]);
