...
public:
  ...
  void addIPF_Plot(const IPF_Plot &v) {
    if(ipf_viewer) ipf_viewer->addIPF_Plot(v);
    if(ipf_viewer_b) ipf_viewer_b->addIPF_Plot(v);
  };
...
private:
  ...
  inline void cb_AltIndexIPF_i(int);
  static void cb_AltIndexIPF(Fl_Widget*, int);
