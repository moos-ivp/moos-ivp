...
bool SBR_Model::setParam(string param, string value, unsigned int pass)
{
  param = tolower(param);
...
bool SBR_Model::handleMsg(string key, double dval, string sval,
...
void SBR_Model::setCurrTime(double new_time)
{
...
vector<VarDataPair> SBR_Model::getMessages(bool clear)
{
  if(!clear)
    return(m_messages);
...
void SBR_Model::print() const
{
  unsigned int i, vsize = m_beacons.size();

  cout << "SBR Model - # Beacons: " << vsize << endl;
  for(i=0; i<vsize; i++)
...
bool SBR_Model::addBeaconBuoy(const string& line)
{
...
bool SBR_Model::handleNodeReport(const string& node_report_str)
{
  NodeRecord new_node_record = string2NodeRecord(node_report_str);

  if(!new_node_record.valid()) {
    addMessage("SBR_DEBUG", "Invalid incoming node report");
    return(false);
  }
...
bool SBR_Model::handleRangeRequest(const string& request)
{
  string vname;
  ...
  if(vname == "") {
    addMessage("SBR_DEBUG", "Range request with null node name");
    return(false);
  }
...
void SBR_Model::addMessage(const string& varname,
			   const string& value)
{
...
void SBR_Model::addMessage(const string& varname, double value)
{
  VarDataPair pair(varname, value);
  m_messages.push_back(pair);
}
...
vector<VarDataPair> SBR_Model::getVisuals()
{
...
void SBR_Model::postBeaconRangePulse(unsigned int ix, string color)
{
...
void SBR_Model::postBeaconRangeReport(unsigned int beacon_ix, 
				      double actual_range, 
				      string vname)
{
  ...
  if((m_report_vars == "short") || (m_report_vars == "both")) {
    ...
    addMessage("SBR_RANGE_REPORT", full_str);
  }

  if((m_report_vars == "long") || (m_report_vars == "both")) {
    vname = toupper(vname);
    addMessage("SBR_RANGE_REPORT_"+vname, str);
  }
  ...
  if((m_rn_algorithm != "") && m_ground_truth) {
    ...
    if((m_report_vars == "short") || (m_report_vars == "both")) {
      string full_str = "vname=" + vname + "," + str;
      addMessage("SBR_RANGE_REPORT_GT", full_str);
    }
    
    if((m_report_vars == "long") || (m_report_vars == "both")) {
      vname = toupper(vname);
      addMessage("SBR_RANGE_REPORT_GT_"+vname, str);
    }
  }
...
double SBR_Model::getTrueBeaconNodeRange(unsigned int beacon_ix, 
...
double SBR_Model::getNoisyBeaconNodeRange(double true_range) const
{
...
void SBR_Model::report(string msg, string term_color)
{
  if(m_verbose)
...
bool SBR_Model::updateNodeRecords(NodeRecord new_record)
{
...
bool SBR_Model::setReachDistance(string str)
{
...
bool SBR_Model::setPingWait(string str)
{
  string left  = biteStringX(str, '=');
  string right = tolower(str); 
...
bool SBR_Model::setPingPayments(string str)
{
...
bool SBR_Model::setReportVars(string str)
{
  string val = tolower(str);
...
bool SBR_Model::setRandomNoiseAlgorithm(string str)
{
...
void SBR_Model::postRangePulse(double x, double y, string color,
