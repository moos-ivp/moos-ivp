...
//-------------------------------------------------------------
// Procedure: buildVarRegistry

bool LogViewLauncher::buildVarRegistry()
{
  return(true);
}
...
bool LogViewLauncher::buildGraphical()
{
  ...
  for(unsigned int k=0; k<m_log_plots.size(); k++) {
    for(unsigned int j=0; j<m_log_plots[k].size(); j++) {
  ...
  for(unsigned int k=0; k<m_vplug_plots.size(); k++) 
