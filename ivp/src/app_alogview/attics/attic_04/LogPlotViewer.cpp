...
void LogPlotViewer::setLeftPlot(const LogPlot& lplot)
{
  m_logplot1 = lplot;
  m_valid_cache = false;
...
void LogPlotViewer::setRightPlot(const LogPlot& lplot)
{
  m_logplot2 = lplot;
  m_valid_cache = false;
...
bool LogPlotViewer::fillCache()
{
  ...
  cout << "m_logplot1 size: " << m_logplot1.size() << endl;
  cout << "m_logplot2 size: " << m_logplot2.size() << endl;

  if((vpsize1 > 0) || (vpsize2 > 0))
    m_valid_cache = true;
  ...
  double min_val1 = m_logplot1.get_min_val();
  double max_val1 = m_logplot1.get_max_val();
  double h_step1 = (h()-m_margin) / (max_val1  - min_val1);

  cout << "m_display_min_time: " << m_display_min_time << endl;
  cout << "m_display_max_time: " << m_display_max_time << endl;

  bool excepted_right = false;
  for(unsigned int i=0; i<vpsize1; i++) {
    double rawt = m_logplot1.get_time_by_index(i);
    //    cout << "raw time: " << rawt << endl;
    ...
    if(add_to_cache) {
      double rawv = m_logplot1.get_value_by_index(i);
  ...
  cout << "cache_x1 size: " << cache_x1.size() << endl;
  cout << "cache_y1 size: " << cache_y1.size() << endl;

  // Otherwise, fill in the cache for the second index
  double min_val2 = m_logplot2.get_min_val();
  double max_val2 = m_logplot2.get_max_val();
  double h_step2 = (h()-m_margin) / (max_val2  - min_val2);
  
  excepted_right = false;
  for(unsigned int j=0; j<vpsize2; j++) {      
    double rawt = m_logplot2.get_time_by_index(j);
    bool add_to_cache = false;
    if((rawt >= m_display_min_time) && (rawt <= m_display_max_time))
      add_to_cache = true;
    else if(rawt <= m_display_min_time) {
    ...
    if(add_to_cache) {
      double rawv = m_logplot2.get_value_by_index(j);
      double scalet = ((rawt - m_display_min_time)  * m_step) + (m_margin/2.0); 
  ...
  cout << "cache_x2 size: " << cache_x2.size() << endl;
  cout << "cache_y2 size: " << cache_y2.size() << endl;
...
{
  cout << "m_valid_cache: " << m_valid_cache << endl;
  if(!m_valid_cache)
    fillCache();
  if(!m_valid_cache)
    return;
