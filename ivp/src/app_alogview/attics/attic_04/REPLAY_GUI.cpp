  ...
  : MarineVehiGUI(g_w, g_h, g_l) 
{
  ...
  m_play_rate = new MY_Output((w()/2)+140, h()-(m_lp_viewer_hgt+25), 70, 20, "play-rate:"); 
  m_play_rate->textsize(info_size); 
  m_play_rate->labelsize(info_size);
...
inline void REPLAY_GUI::cb_LeftLogPlot_i(int index) {
  cout << "Getting...." << endl;
  LogPlot logplot = m_dbroker.getLogPlot(index);
  cout << "Got it!...." << logplot.size() << endl;
  lp_viewer->setLeftPlot(logplot);
...
inline void REPLAY_GUI::cb_RightLogPlot_i(int index) {
  cout << "RGetting...." << endl;
  LogPlot logplot = m_dbroker.getLogPlot(index);
  cout << "RGot it!...." << logplot.size() << endl;
  lp_viewer->setRightPlot(logplot);
...
void REPLAY_GUI::updateXY() 
{
  ...
  m_disp_time->value(dtime.c_str());
   ...
   // Play Rate
  if(m_stream) {
    if(m_step_time_ix == 0) m_play_rate->value("0.125Hz");
    else if(m_step_time_ix == 1) m_play_rate->value("0.25Hz");
    else if(m_step_time_ix == 2) m_play_rate->value("0.5Hz");
    else if(m_step_time_ix == 3) m_play_rate->value("1Hz");
    else if(m_step_time_ix == 4) m_play_rate->value("2Hz");
    else if(m_step_time_ix == 5) m_play_rate->value("4Hz");
    else if(m_step_time_ix == 6) m_play_rate->value("8Hz");
    else if(m_step_time_ix == 7) m_play_rate->value("16Hz");
    else if(m_step_time_ix == 8) m_play_rate->value("32Hz");
  }
  else
    m_play_rate->value("Paused");
...
void REPLAY_GUI::resizeDataText(int wsize) 
{
  ...
  m_play_rate->labelsize(wsize);
  m_play_rate->textsize(wsize);
}
...
void REPLAY_GUI::resizeWidgets() 
{
  ...
  // LogPlot1
  m_label1->resize(dw*0.02, row4, dw*0.07, fld_hgt);
  m_curr1->resize(dw*0.14, row4, dw*0.15, fld_hgt);
  m_low1->resize(1, row1, dw*0.07, fld_hgt);
  m_high1->resize(1, row3, dw*0.07, fld_hgt);
  // LogPlot2
  m_label2->resize(dw*0.74, row4, dw*0.07, fld_hgt);
  m_curr2->resize(dw*0.85, row4, dw*0.15, fld_hgt);
  m_low2->resize(dw*0.93, row1, dw*0.07, fld_hgt);
  m_high2->resize(dw*0.93, row3, dw*0.07, fld_hgt);
  // Time Min/Max
  ...
  m_play_rate->resize(dw*0.62, row4, dw*0.09, fld_hgt);
