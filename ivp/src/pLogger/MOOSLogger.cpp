...
CMOOSLogger::CMOOSLogger()
{
    ...
    //m_SynchronousLogVars.resize(100);
...
bool CMOOSLogger::OnNewMail(MOOSMSG_LIST &NewMail)
{
    ...
    if(m_Comms.PeekMail(NewMail,"LOGGER_RESTART",Msg))
    {
  ...
  AddMOOSVariable("MOOS_DEBUG","MOOS_DEBUG","",0);
    ...
    if(m_MissionReader.GetConfigurationParam("SYNCLOG",sTmp))
    {
        ...
        if(!sTmp.empty())
        {
            ...
            if(m_dfSyncLogPeriod<0.1)
            {
                m_dfSyncLogPeriod = 0.1;
            }
        }
    }
    ...
    ConfigureLogging();       // mikerb changed Jun2207 
    //if(!ConfigureLogging())
    //    return false;
...
bool CMOOSLogger::DoSyncLog(double dfTimeNow)
{
    ...
    for(int nVar = 0; nVar<m_SynchronousLogVars.size();nVar++)
    {
