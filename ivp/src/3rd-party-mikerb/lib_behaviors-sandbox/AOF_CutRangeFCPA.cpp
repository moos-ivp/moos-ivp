...
AOF_CutRangeFCPA::AOF_CutRangeFCPA(IvPDomain gdomain)
  : AOF(gdomain)
{
...
bool AOF_CutRangeFCPA::setParam(const string& param, double param_val)
{
  if(param == "oslat") {
    os_lat = param_val;
    os_lat_set = true;
...
bool AOF_CutRangeFCPA::initialize()
{
...
double AOF_CutRangeFCPA::evalBox(const IvPBox *b) const
{
  double eval_crs = 0;
  double eval_spd = 0;
  double eval_tol = tol;
...
double AOF_CutRangeFCPA::metric(double gval) const
{
