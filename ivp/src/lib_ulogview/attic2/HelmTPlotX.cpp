...
HelmTPlotX::HelmTPlotX() : TimePlot()
{
  m_vehi_type = "unknown";
  m_vehi_length = 0;
}
...
void HelmTPlotX::setVehiType(string str)
{
...
bool HelmTPlotX::addEntry(double gtime, string gval)
{
...
string HelmTPlotX::getValueByIndex(const string& qtype, 
...
string HelmTPlotX::getValueByTime(const string& qtype, double gtime) const
{
...
string HelmTPlotX::getValueByTimeUTC(const string& qtype, double gtime) const
{
...
double HelmTPlotX::getTimeByIterAdd(double ctime, 
...
double HelmTPlotX::getTimeByIterSub(double ctime, 
				   unsigned int iter_offset) const
{
  unsigned int curr_index = getIndexByTime(ctime);
  unsigned int curr_iter  = m_helm_reports[curr_index].getIteration(); 
...
unsigned int HelmTPlotX::getIterByTime(double gtime) const
{
...
unsigned int HelmTPlotX::getIterByIndex(unsigned int index) const
{
...
unsigned int HelmTPlotX::getIterByTimeUTC(double gtime) const
{
  unsigned int index = getIndexByTimeUTC(gtime);
  return(m_helm_reports[index].getIteration());
}
...
unsigned int HelmTPlotX::getIndexByIter(unsigned int iter) const
{
