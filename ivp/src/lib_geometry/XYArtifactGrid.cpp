...
XYSquare XYArtifactGrid::getElement(int ix) const
{
	XYSquare retElement;
...
void XYArtifactGrid::setClearance(int index, double val)
{
...
double XYArtifactGrid::getClearance(int index) const
{
	if ((index >= 0) && (index < clearances.size())){
...
void XYArtifactGrid::setArtVec(int index, const ArtVec & artvec)
{
...
ArtVec XYArtifactGrid::getArtVec(int index) const
{
	if ((index >= 0) && (index < artifacts.size())){
...
uint XYArtifactGrid::getDetectedArts() const
{
	...
	for(std::vector<ArtVec>::const_iterator p = artifacts.begin(); p != artifacts.end(); p++){
		count += p->size();
	}
...
bool XYArtifactGrid::processDelta(const string& str)
{
	...
	for(int i=0; i<vsize; i++) { // For each index,old_clearance,new_clearance
