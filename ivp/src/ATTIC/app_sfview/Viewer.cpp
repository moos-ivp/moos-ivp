...
void Viewer::drawAOF()
{
  ...
  IvPBox universe = aof->getUniverse();
  int dim = universe.getDim();
  if((dim != 2) && (dim != 3))
    return;
  
  int xmin = universe.pt(0,0);
  int xmax = universe.pt(0,1);
  int ymin = universe.pt(1,0);
  int ymax = universe.pt(1,1);
...
#if 1
  reflector.create_uniform(&unifbox, &unifbox);
#endif
#if 0
  reflector.create_uniform(&unifbox, &unifbox, 10);
  reflector.create_priority(1000, 0.01);
#endif
