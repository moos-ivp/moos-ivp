...
bool ULH_MOOSApp::OnNewMail(MOOSMSG_LIST &NewMail)
{
...
bool ULH_MOOSApp::OnConnectToServer()
{
  registerVariables();
  return(true);
}
...
void ULH_MOOSApp::postConnectionPairs()
{
  m_pending_pairs = false;
...
void ULH_MOOSApp::registerVariables()
{
...
void ULH_MOOSApp::handlePendingGUI()
{
  if(!m_gui)
    return;
  
  string this_app_name = GetAppName();
  PostingBuffer pbuffer = m_gui->hsm.getPostingBuffer();
...
void ULH_MOOSApp::handleNewMail(const MOOS_event & e)
{
  string this_app_name = GetAppName();
  ...
  for (size_t i = 0; i < e.mail.size(); ++i) {
	...
	m_gui->setCurrTime(alogview_time);
	cout << "T" << flush;
	handled = true;
      }
      if(handled)
...
void ULH_MOOSApp::handleIterate(const MOOS_event & e) 
{
#if 0 // mikerb
...
void ULH_MOOSApp::handleStartUp(const MOOS_event & e) 
{
  STRING_LIST sParams;
