...
ULV_GUI *Launcher::launch(int argc, char **argv)
{
  MBTimer total_timer;
  total_timer.start();
...
void Launcher::checkForMinMaxTime(int argc, char **argv)
{
  int i;
  for(i=1; i<argc; i++) {
    string argi = argv[i];
...
void Launcher::setBackground(int argc, char **argv)
{
...
void Launcher::setSizeOfGUI(int argc, char **argv)
{
...
void Launcher::setWindowLayout(int argc, char **argv)
{
...
void Launcher::setALogFiles(int argc, char **argv)
{
  for(int i=1; i<argc; i++)
...
bool Launcher::setALogFileSkews()
{
  MBTimer parse_timer;
  parse_timer.start();
...
bool Launcher::parseALogFile(unsigned int index)
{
...
void Launcher::determineVehicleNames()
{
  MBTimer parse_timer;
  parse_timer.start();
...
bool Launcher::buildGraphical()
{
  ...
  for(k=0; k<m_log_plots.size(); k++) {
    for(j=0; j<m_log_plots[k].size(); j++) {
      ...
      if(m_log_plots[k][j].getVarName() == "NAV_X")
	m_gui->np_viewer->addLogPlotNAVX(m_log_plots[k][j]);
      else if(m_log_plots[k][j].getVarName() == "NAV_Y")
	m_gui->np_viewer->addLogPlotNAVY(m_log_plots[k][j]);
      else if(m_log_plots[k][j].getVarName() == "NAV_HEADING")
