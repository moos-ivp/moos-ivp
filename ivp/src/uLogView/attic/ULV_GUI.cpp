  ...
  : MarineVehiGUI(g_w, g_h, g_l) 
{
  ...
  m_but_zoom_in_time = new MY_Repeat_Button(time_pos+120, 
					    h()-(m_lp_viewer_hgt+26), 
					    30, 21, "IN");
  m_but_zoom_out_time = new MY_Repeat_Button(time_pos+160, 
					     h()-(m_lp_viewer_hgt+26), 
					     35, 21, "OUT");
  m_but_zoom_reset_time = new MY_Button(time_pos+200, 
					h()-(m_lp_viewer_hgt+26), 
					45, 21, "RESET");
...
void ULV_GUI::augmentMenu()
{
  ...
  mbar->add("Replay/Step by Seconds",  0, 
	    (Fl_Callback*)ULV_GUI::cb_StepType, (void*)0, 
	    FL_MENU_RADIO|FL_MENU_VALUE);
  mbar->add("Replay/Step by Helm Iterations",  0, 
	    (Fl_Callback*)ULV_GUI::cb_StepType, (void*)1, 
	    FL_MENU_RADIO|FL_MENU_DIVIDER);
...
//----------------------------------------- StepType
inline bool ULV_GUI::cb_StepType_i(int val) {
  if(val == 0)
...
void ULV_GUI::cb_StepType(Fl_Widget* o, int v) {
  int val = (int)(v);
  ((ULV_GUI*)(o->parent()->user_data()))->cb_StepType_i(val);
}
...
void ULV_GUI::updateXY() 
{
  ...
  string dtime = doubleToString(ctime, 2);
...
void ULV_GUI::addLogPlot(const LogTPlot& logplot)
{
  ...
  if(vname != "") {
    labelA = "LogPlots(L)/" + vname + "/" + logplot.getVarName();
    labelB = "LogPlots(R)/" + vname + "/" + logplot.getVarName();
  }
  else {
    labelA = "LogPlots(L)/" + logplot.getVarName();
    labelB = "LogPlots(R)/" + logplot.getVarName();
  }
