...
double LogPlotViewer::get_curr_val1(double gtime)
{
  if(m_logplots.size() == 0)
    ...
    return(m_logplots[view_index1].getValueByTime(gtime));
}
...
double LogPlotViewer::get_curr_val2(double gtime)
{
  if(m_logplots.size() == 0)
    ...
    return(m_logplots[view_index2].getValueByTime(gtime));
}
...
bool LogPlotViewer::fillCache()
{
  ...
  for(i=0; i<vpsize1; i++) {
    double rawt = vplot1.getTimeByIndex(i);
  ...
  for(j=0; j<vpsize2; j++) {      
    double rawt = vplot2.getTimeByIndex(j);
    bool add_to_cache = false;
    if((rawt >= m_display_min_time) && (rawt <= m_display_max_time))
      add_to_cache = true;
    else if(rawt <= m_display_min_time) {
