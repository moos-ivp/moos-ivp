...
#define CCLMAXINT     (2147483647)  // 2^31-1
...
struct CCL_contactxy_t
{
	...
	double y;
	float heading;            // observer heading for relative bearings
...
struct CCL_nav_t
{
	...
	double lat;                // beacon pos'n (lat) 
	double lon;                // beacon pos'n (lon)
	float cep;                 // CEP - error (cm)
	float c_sound;             // speed of sound (m/s)
    float gpsstd;              // GPS Std deviation
	...
	int gps_hdop;              // dilution of precision
} ;
...
void CCL_RxNavFrame(sDataFrame *frame,CCL_nav_t *s);
...
sDataFrame *CCL_MakeStateXYFrame(int src,int dest,int ack,CCL_statexy_t *sStateXY);
sDataFrame *CCL_MakeNavXYFrame(int src,int dest,int ack,CCL_navxy_t *sNavXY);
sDataFrame *CCL_MakeNavFrame(int src,int dest,int ack,CCL_nav_t *sNav);
sDataFrame *CCL_MakeContactXYFrame(int src,int dest,int ack, CCL_contactxy_t *sContact);
