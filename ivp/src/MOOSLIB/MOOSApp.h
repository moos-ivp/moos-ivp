...
public:
    ...
    */
    ...
    /** Called when the class has disconnects from  the server. Put code you want to run when this happens in
    a a virtual version of this method*/
...
protected:
    ...
    /** The MOOSComms node. All communications happens by way of this object. You'll often do things like
    m_Comms.Notify("VARIABLE_X","STRING_DATA",dfTime) top send data */
	...
	double GetAppStartTime();
   	...
   	/**a very useful object that lets us retrieve configuration information from the 
	mission file using calls like ::GetConfigurationParam() */
    ...
    /** Add a dynamic (run time) variable 
        ...
        @param sPublishName  if you call PublishFreshMOOSVariables() (and you've written to the dynamic varible since
        the last call) the variable will be published under this name.
        @param CommsTime - if sSubscribeName is not empty this is the minimum time between updates which you are interested in knowing
        about, so if CommsTime=0.1 then the maximum update rate you will see on the variable from the DB is 10HZ.
    */
    ...
    /** Pass mail (usually collected in OnNewMail) to the set of dynamic variables. If they are interested (mail name matches their subscribe name)
    they will uypdate themselves automatically */
