...
LauncherULI::LauncherULI() : GeneralLauncher()
{
  m_gui_width  = 415;
  m_gui_height = 400;
  m_gui        = 0;
...
ULI_GUI *LauncherULI::launch(int argc, char **argv)
{
  MBTimer total_timer;
  total_timer.start();
...
void LauncherULI::setSizeOfGUI(int argc, char **argv)
{
...
bool LauncherULI::parseALogFile()
{
  ...
  while(!done) {
	...
	if(var == "BHV_IPF")
	  m_entries_ipf_plot.push_back(entry);
      }
    }
  }
...
bool LauncherULI::buildPlotsAndGUI()
{
  MBTimer parse_timer;
  parse_timer.start();
  cout << "Refining alog data to build IPF_Plots..." << endl;

  Populator_IPF_TPlot pop_ipf;
  pop_ipf.setVName(m_vname);
  pop_ipf.setIvPDomain(m_ivp_domain);
  pop_ipf.setLogStartUTC(m_log_starts[0]);

  bool ok = pop_ipf.populateFromEntries(m_entries_ipf_plot);
  parse_timer.stop();
  if(!ok) {
    cout << termColor("red") << "Unable to build IPF Plots!!!" << endl;
    cout << "(Likely due to lack of BHV_IPF entries in .alog file)";
  ...
  //-------------------------------------------------------------
  string title = "uLogViewIPF-" + m_vname;
  m_gui = new ULI_GUI(m_gui_width, m_gui_height, title.c_str());
  if(!m_gui)
    return(false);
  ...
  for(k=0; k<ksize; k++) {
    const IPF_TPlot& ipf_plot = pop_ipf.getPlotIPF(k);
    m_gui->addIPF_Plot(ipf_plot);
  }

  const IterTPlot& iter_plot = pop_ipf.getIterPlot();
  m_gui->ipf_viewer->setIterPlot(iter_plot);
  
  m_gui->updateXY();
  m_gui->ipf_viewer->setMinMaxTimeUTC(m_global_mintime_utc,
