...
#if 0
  cout << "Attractor Pre-Normalize MIN-WT: " << ipf->getPDMap()->getMinWT() << endl;
  cout << "Attractor Pre-Normalize MAX-WT: " << ipf->getPDMap()->getMaxWT() << endl;
#endif

  ipf->getPDMap()->normalize(0.0, 100.0);

#if 0
  cout << "Attractor MIN-WT: " << ipf->getPDMap()->getMinWT() << endl;
  cout << "Attractor MAX-WT: " << ipf->getPDMap()->getMaxWT() << endl;
#endif
				  ...
				  double cnX, double cnY)
{
  ...
  double pct = exp_wgt*(1 + strength * (range_max-dist) / total_range);
  return(pct);
}
