...
BHV_GliderWaypoint::BHV_GliderWaypoint(IvPDomain gdomain) : 
  IvPBehavior(gdomain)
{
  // First set variables at the superclass level
  m_descriptor = "bhv_beatcurrent";  
...
bool BHV_GliderWaypoint::setParam(string param, string val) 
{
...
void BHV_GliderWaypoint::onRunToIdleState() 
//void BHV_GliderWaypoint::onIdleState() 
{
...
IvPFunction *BHV_GliderWaypoint::onRunState() 
{
...
bool BHV_GliderWaypoint::updateInfoIn()
{
  bool ok1, ok2, ok3;
...
bool BHV_GliderWaypoint::setNextWaypoint()
{
  ...
  if(m_lead_distance >= 0) {
    ...
    if(track_anchor) {
      ...
      if(m_lead_damper > 0) {
	dist_to_trackline = hypot((nx-m_osx),(ny-m_osy));
	...
	dist = m_lead_distance * damper_factor;  
      ...
      // Compensate for current current_x, current_y
      // HS 092509

      double ds = hypot(dist, dist_to_trackline);
      double dt = ds/m_cruise_speed;
      ...
      m_water_speed = m_cruise_speed - m_current_x * sin(hdg_ang) - m_current_y * cos(hdg_ang);
...
IvPFunction *BHV_GliderWaypoint::buildOF(string method) 
{
  IvPFunction *ipf = 0;
  ...
  if((method == "roc") || (method == "rate_of_closure")) {
    ...
    ok = ok && aof_wpt.setParam("desired_speed", m_water_speed);
...
void BHV_GliderWaypoint::postStatusReport()
{
  int    current_waypt = m_waypoint_engine.getCurrIndex();
  int    waypt_cycles  = m_waypoint_engine.getCycleCount();
...
void BHV_GliderWaypoint::postViewableSegList()
{
...
void BHV_GliderWaypoint::postErasableSegList()
{
  XYSegList seglist = m_waypoint_engine.getSegList();
  seglist.set_label(m_us_name + "_" + m_descriptor);
...
void BHV_GliderWaypoint::postCycleFlags()
{
...
void BHV_GliderWaypoint::handleVisualHint(string hint)
{
