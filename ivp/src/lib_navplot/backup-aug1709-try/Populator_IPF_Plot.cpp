...
void Populator_IPF_Plot::handleEntry(double ipf_time, 
				     const string& ipf_str)
{
  // The created IvPFunction instance will be passed to the
  // the IPF_Plot and it will own it.
  IvPFunction *ipf = StringToIvPFunction(ipf_str);

  string context = ipf->getContextStr();

  string       ipf_source;
  unsigned int ipf_iter;

  // The Context string might come in the form of NUM:CONTEXT"
  vector<string> svector = parseString(context, ':');
  if(svector.size() == 2) {
    ipf_iter = (unsigned int)(atoi(svector[0].c_str()));
    ipf_source = svector[1];
  }
  string ipf_tag = m_vname + "_" + ipf_source;

  int index = -1;
  int vsize = m_ipf_tags.size();
  for(int i=0; i<vsize; i++) {
    if(m_ipf_tags[i] == ipf_tag)
      index = i;
  }
  ...
  if(index == -1) {
    ...
    m_ipf_tags.push_back(ipf_tag);
    m_ipf_plots.push_back(new_plot);
    index = vsize;
  }

  m_ipf_plots[index].addEntry(ipf, ipf_time, ipf_iter);
}
